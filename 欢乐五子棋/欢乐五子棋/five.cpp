...
int  step[21][21] = { 0 };//当元素的值为1时表示黑子、当元素的值为0时表示白子
int  curX = 21, curY = 10;    //分别表示光标位置的x和y，初始值默认在棋盘中心
int  player = 0;  //在网络中，用0和1表示玩家可否下子
int  k;//用于接收返回的键值
int  tot;//记录棋子步数
...
int Int_game()
{
	...
	while (lian)//重复连接服务端
	{
		...
		//判断套接字创建是否成功，常量INVALID_SOCKET表示无效套接字
		...
		serverAdd.sin_addr.S_un.S_addr = inet_addr(SERVER_IP);
		if (lian == 2)
		{
			system("PAUSE");
			printf("重新连接中……\n");
		}
		if (connect(sConnection, (struct sockaddr*)(&serverAdd), sizeof(serverAdd)) == SOCKET_ERROR)
		{
			printf("与服务端连接失败！\n");
			lian = 2;
		}
	...
	while (1)
	{
		gotoxy(1, 21);         //为了使提示在该区域显示
		printf("请等待...\n");
		recv(sConnection, buff, 100, 0);//接收信息
		switch (buff[0])
		{
		case 'N'://判断对方是否赢
			curX = buff[1];
			...
			gotoxy(curX, curY);
			printf("●");
			step[curY - 1][(curX + 1) / 2 - 1] = -1;
			//xiazi_music();//调用下子音乐
			recorde_completor++;//步数+1
			...
			if ((check1(curY - 1, (curX + 1) / 2 - 1)) == 1)//判断输赢
			{
				...
				closesocket(sConnection);    //关闭连接套接字
				WSACleanup(); //终止Windows Socket在所有线程的操作
				gotoxy(1, 24);
		...
		while (player == 1)
		{
			...
			case ENTER:
				if (step[curY - 1][(curX + 1) / 2 - 1] == 0)
				{
					...
					buff[2] = curY;
					buff[3] = '\0';
					send(sConnection, buff, 4, 0);
					//xiazi_music();//调用下子音乐
					recorde_host++;//我下棋步数加1;
					//play_chess(3, 0, curX, curY, q);//棋盘的录制
					player = 0;
					if ((check(curY - 1, (curX + 1) / 2 - 1)) == 1)//判断自己是否会赢
					{
						finish = clock();//计时结束
						recorde_host = recorde_completor;////步数赋值
						show_gl();//当五子连成一线时，打印五个星
			...
			case C:
				...
				gotoxy(cX + 3, cY++);
				scanf_s("%s", buff);
				send(sConnection, buff, sizeof(buff), 0);
				break;
			case ESC:
				buff[0] = 'E';
				buff[1] = '\0';
...
void xiazi_music()
{
	PlaySound("xiazi.wav", NULL, SND_FILENAME);
}

//对手胜利音乐
void ManWin_music()
{
	PlaySound("ManWin.wav", NULL, SND_FILENAME);
}

//玩家胜利音乐
void FemaleWin_music()
{
	PlaySound("FemaleWin.wav", NULL, SND_FILENAME);
}
*/
//重新开始游戏
int rebegin()
...
void playsound()
{
	...
	int choose = -1;
		...
		case 2:
		{
				  PlaySound(TEXT("2.wav"), 0, SND_FILENAME);
